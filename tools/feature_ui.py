...
def _feats_of(g):
    """Фичи группы: новый формат или фолбэк из legacy-полей."""
    if "features" in g:
        return g.get("features") or {}
    f = {}
    if g.get("on") or g.get("off"):
        f["schedule"] = {}
    if g.get("motion_sensor"):
        f["motion"] = {"timeouts": g.get("motion_timeouts"),
                       "no_night_auto": g.get("no_night_auto_flag")}
    if g.get("nightlight"):
        f["nightlight"] = g["nightlight"]
    return f
...
def ui_motion(g, gid):
    ...
    cards = [_title("📡 Движение"),
             _sel("input_select.light_%s_motion_sensor" % gid, "Датчик"),
    ...
    if mo.get("no_night_auto"):
        cards.append(_bool(mo["no_night_auto"], "Ночью авто — выкл"))
    if mo.get("timeouts") == "own":
        cards.append(_grid([
            _num("input_number.light_%s_motion_day_min" % gid, "Таймаут день"),
...
FEATURE_UI = {"party": ui_party, "dusk": ui_dusk, "ct": ui_ct, "imitation": ui_imitation,"schedule": ui_schedule, "motion": ui_motion, "nightlight": ui_nightlight}
