...
def _lg_decide(g, cfg):
    ...
    if sel_on == "Датчик движения":
        if not ms:
            return {"on": False}
        # Nightlight активен когда: vecher=on + feature_nightlight=on
        nightlight_active = (
            night
            and _lg_state("input_boolean.feature_%s_nightlight" % gid) == "on"
        )
        if nightlight_active and presence:
            return {"on": True, "nightlight": True}
        return {"on": presence}
    ...
    if sel_off == "Рассвет":
        ...
        if off_min is not None and not presence:
