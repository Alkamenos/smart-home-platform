...
async def test_survive_restart(tmp_path):
    ...
    engine1 = FSMEngine()
    engine1.register_definition(
        FSMDefinition("light.test", "OFF", ("OFF", "ON"), (Transition("OFF", "ON", "manual"),))
    )
    ...
    if storage.exists():
        ...
        if entity_id in saved_state:
            engine2.reset_state(entity_id, saved_state[entity_id]["current_state"])
