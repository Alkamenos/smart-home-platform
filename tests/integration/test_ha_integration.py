...
class SimpleHAWebSocketClient:
    """Minimal WebSocket client for Home Assistant."""
...
class MockFSMEngine:
    ...
    async def trigger(self, entity_id: str, event: str, external_ctx: dict[str, Any]) -> None:
        ...
        await self.process_event('state_change', {
            'entity_id': entity_id,
...
async def test_ha_integration_full_cycle(ha_container, ha_token, mock_engine):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            test_light = "light.kitchen" if "light.kitchen" in existing_ids else (
                "light.bedroom" if "light.bedroom" in existing_ids else None
            )
            ...
            assert command['domain'] == 'light', f"Expected 'light' domain, got '{command['domain']}'"
            assert command['service'] == 'turn_on', f"Expected 'turn_on' service, got '{command['service']}'"
...
async def test_light_turn_on_command_sent_to_ha(ha_container, ha_token, ha_session):
    ...
    async with ha_session.post(
        f"{http_url}/api/services/light/turn_on",
        headers=headers,
        json=service_data
    ) as resp:
        if resp.status == 401:
