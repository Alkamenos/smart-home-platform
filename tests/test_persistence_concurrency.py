...
async def save_state_async(persistence: StatePersistence, entity_id: str, state: str, context: dict):
    """
...
async def test_concurrent_save_state():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        tasks = [
            ...
            for entity_id in expected_data.keys()
        ]
        ...
        for entity_id, expected in expected_data.items():
            assert entity_id in saved_data, \
                f"Entity '{entity_id}' is missing from saved data"
...
async def test_concurrent_same_entity():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert entity_id in saved_data, \
            f"Entity '{entity_id}' should exist after concurrent writes"
        ...
        print(f"✓ Concurrent writes to same entity handled correctly - no corruption")
        return True
