...
## Helpers логирования (с v1.2.0)
- `input_select.loglevel_platform` — уровень логирования платформы (Выкл/Ошибки/Предупреждения/Инфо/Отладка).
- `input_select.loglevel_lighting` — уровень логирования освещения.
- `input_select.loglevel_climate` — уровень логирования климата.
- `input_select.loglevel_ventilation` — уровень логирования вентиляции.
- `input_select.loglevel_sensor_health` — уровень логирования датчиков.
- `input_select.loglevel_covers` — уровень логирования штор.
- Initial значения берутся из манифеста (`logging.level`, `logging.modules`), иначе «Инфо».
- Манифест остаётся фолбэком, источник правды — helper.
- Добавлены в whitelist cleanup (не считаются сиротами).
