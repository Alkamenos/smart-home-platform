...
class FileStateStore(StateStore):
        ...
        try:
            if self._path.exists() and self._path.stat().st_size > 0:
                with open(self._path) as f:
                    ...
                    if self._logger:
                        self._logger.info(f"Loaded {len(self._cache)} states from {self._path}")
            else:
                if self._logger:
    ...
    async def _save_to_file(self) -> None:
        ...
        try:
            ...
            if self._logger:
                self._logger.debug(f"Saved {len(self._cache)} states to {self._path}")
...
class InputTextStateStore(StateStore):
    ...
    def _deserialize(self, data: str) -> dict | None:
    ...
    async def delete(self, entity_id: str) -> bool:
        ...
        try:
            if hasattr(self, "state") and hasattr(self.state, "set"):
                ...
                await self._hass.services.async_call(
                    "input_text", "set_value", {"entity_id": helper_name, "value": ""}
                )
                return True
        except Exception as e:
            if self._logger:
