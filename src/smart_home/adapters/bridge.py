...
class ActionBridge:
    ...
    def _on_fsm_transition(self, data: dict) -> None:
        ...
        if command is None:
            ...
            self._logger.debug(
                f"No physical action for state {to_state}", entity_id=entity_id, to_state=to_state
            )
            return
    ...
    def _send_command(
        self, entity_id: str, command: str, attributes: dict, from_state: str, to_state: str
    ) -> None:
        """
...
class StateSync:
    ...
    def _on_ha_state_change(self, data: dict) -> None:
        ...
        if expected_physical != actual_physical:
            ...
            self._fsm_engine.trigger(
                entity_id,
                "manual_change",
                {"physical_state": actual_physical, "ha_state": new_state, "reason": "State sync"},
            )
    ...
    async def sync_all_at_startup(self, adapter: HomeAssistantAdapter) -> None:
        ...
        try:
            ...
            for entity in entities:
                ...
                if expected != actual:
                    self._logger.info(
                        f"Startup sync: {entity_id}", fsm_state=fsm_state.current, ha_state=ha_state
                    )
                    ...
                    self._fsm_engine.trigger(
                        entity_id,
                        "sync_state",
                        {"physical_state": actual, "ha_state": ha_state, "reason": "Startup sync"},
                    )
...
def setup_integration_bridge(
    event_bus: EventBus, fsm_engine: FSMEngine, adapter: HomeAssistantAdapter, logger: Logger
