...
class DashboardIntegration:
    ...
    async def create_fsm_sensor(self, fsm_id: str, name: str, feature_type: str) -> bool:
        """
    ...
    async def _update_fsm_sensor(self, fsm_id: str, state: str, event_data: dict[str, Any]):
        ...
        # Обновляем состояние
        await self.ha.set_entity_state(entity_id=entity_id, state=state, attributes=attributes)

    async def create_status_binary_sensor(self, fsm_id: str, name: str) -> bool:
        """
    ...
    async def create_dashboard_view(
        self, title: str = "Smart Home Automation", url_path: str = "smart-automation"
    ) -> bool:
        """
    ...
    def _generate_dashboard_cards(self) -> list[dict[str, Any]]:
        ...
        for fsm_id, entity_id in self._created_entities.items():
            if entity_id.startswith("sensor."):
                ...
                cards.append({"type": "entity", "entity": entity_id, "name": f"FSM {fsm_id}"})
