...
class SensorSimulator:
    ...
    def get_next_value(self, simulation_time: datetime) -> Any:
        ...
        if pattern == "constant":
            ...
            elapsed = (simulation_time - self.last_update).total_seconds() if self.last_update else 0
...
class DigitalTwin:
    ...
    async def start(self, mode: SimulationMode = SimulationMode.REAL_TIME, 
    ...
    async def _run_real_time(self) -> None:
        ...
        for simulator in self.sensor_simulators.values():
            if simulator.last_update is None:
                simulator.last_update = self.state.current_time - timedelta(seconds=simulator.config.update_interval)
    ...
    async def _process_step(self) -> List[SimulationEvent]:
        ...
        for sensor_id, simulator in self.sensor_simulators.items():
            ...
            last_update = simulator.last_update or (self.state.current_time - timedelta(seconds=simulator.config.update_interval + 1))
    ...
    def _finalize_report(self) -> None:
        ...
        self._report.duration_real = (self._report.end_time - self._report.start_time).total_seconds()
        
        if self.state.start_time and self.state.end_time:
            self._report.duration_simulated = (self.state.end_time - self.state.start_time).total_seconds()
