...
- **Полная миграция освещения на FSM**: все 16 групп освещения используют FSM напрямую
  - Концепция `fsm_shadow` удалена как избыточная — FSM исполняется сразу при `fsm_enabled: true`
...
- `FSM_SPEC.md`: обновлён статус — shadow mode удалён

### Удалено
- Концепция `fsm_shadow` (теневой режим) — признана избыточной
