...
## [2026-09-06] fix: надёжная регистрация watchdog-мапперов и FSM климата
- независимые startup-циклы регистрации (append-only, без splice-вставок)

## [2026-09-06] Generic FSM-watchdog синхронизации с устройствами
- fsm_engine: `fsm_register_sync` / `fsm_sync_tick` — мапперы реального состояния, grace-период, авто-устранение расхождений (цикл 60с)
- covers/ventilation: feature-specific синхронизации заменены регистрацией мапперов
- climate: гарантированная регистрация FSM всех зон до проверки датчиков
# CHANGELOG
...
## 2026-09-03 — FSM: Интеграция контекста комнаты в шторы

### Изменено
- `features/covers/runtime.py`: интеграция контекста комнаты
  - `_cv_get_room_context()`: чтение состояния автомата комнаты с fallback на прямые флаги
  - `_cv_tick()`: использует контекст комнаты вместо прямой проверки `my_doma`
  - `_cv_decide_cover()`: учёт вечеринки — шторы не закрываются в режиме `PARTY`
  - `_cv_immediate_close_on_leave()`: использует контекст комнаты

### Семантика
- Если автомат комнаты зарегистрирован: контекст берётся из `fsm_get_state("room.main")`
- Если автомат комнаты НЕ зарегистрирован: fallback на прямые флаги `my_doma`, `dogs_home`, `party_mode`
- В режиме вечеринки (`PARTY`): шторы не закрываются по расписанию (возвращается `keep`)
- В режиме сна (`SLEEPING`): считается как ночь для целей закрытия штор

### Обратная совместимость
- Если автомат комнаты не зарегистрирован, используется старая логика
- Флаг `dogs_home` пока остаётся отдельным (не входит в автомат комнаты)
- Существующие хелперы и сервисы не затронуты


## 2026-09-03 — FSM: Режим сна (SLEEPING) для автомата комнаты

### Добавлено
- Хелпер `input_boolean.sleep_mode`: ручной переключатель режима сна
- Триггер `_room_trigger_sleep()`: обработка включения/выключения режима сна
- `@state_trigger` на `input_boolean.sleep_mode`: мгновенная реакция на изменение

### Изменено
- `features/room/fsm.py`: расширены переходы автомата комнаты
  - `sleep_on`: теперь можно включить сон из `PARTY`
  - `party_on`: теперь можно включить вечеринку из `SLEEPING`
  - `sleep_off_night`: пробуждение ночью возвращает в `HOME_NIGHT`
- `tools/gen_helpers.py`: добавлен `sleep_mode` в глобальные хелперы
- `features/room/runtime.py`: добавлен `_room_trigger_sleep` и `@state_trigger`

### Семантика режима сна
- Включение: `input_boolean.sleep_mode → on` → комната переходит в `SLEEPING`
- Выключение: `input_boolean.sleep_mode → off` → комната возвращается в `HOME_DAY` или `HOME_NIGHT` (в зависимости от времени суток)
- В режиме сна: шторы закрыты, свет не включается по датчику движения (будет добавлено в Фазе 4)
- Режим сна можно включить из любого состояния (включая `PARTY`)
- Вечеринку можно включить из режима сна (приоритет вечеринки ниже, чем сна)

### Использование
```
# Включить режим сна
Вызвать сервис: input_boolean.turn_on
Параметры: entity_id: input_boolean.sleep_mode

# Выключить режим сна (пробуждение)
Вызвать сервис: input_boolean.turn_off
Параметры: entity_id: input_boolean.sleep_mode

# Проверить состояние комнаты
Developer Tools → States → sensor.room_context
```


## 2026-09-03 — FSM: Агрегированный сенсор автоматов

### Добавлено
- `sensor.fsm_overview`: агрегированное состояние всех автоматов
  - Атрибут `count`: количество зарегистрированных автоматов
  - Атрибут `states`: словарь {entity_id: state} для всех автоматов
  - Атрибут `updated_at`: время последнего обновления
- `_fsm_publish_overview()`: публикация обзора автоматов (вызывается каждые 60 сек)
- `fsm_publish_all()`: принудительная публикация состояний всех автоматов

### Использование
```
# Проверить состояние всех автоматов
Developer Tools → States → sensor.fsm_overview
Атрибуты:
count: 5
states: {"room.main": "HOME_DAY", "cover.bedroom": "CLOSED", ...}

# Принудительная публикация
Вызвать сервис: pyscript.fsm_publish_all
```

### Следующие шаги
- Добавить карточку автоматов на дашборд настроек
- Интеграция контекста комнаты в освещение (Фаза 4)
